...
def setupPreview(root,master,lbl):
	...
	tk.Button(root,text="Scan",font=("Courier",12),command=lambda:updateState(master,'Capture')).grid(row=2,column=2,padx=10,pady=10)
...
def configSettings(master):
	...
	confirm = tk.Button(root,text="Update Settings",font=("Courier",12),command=lambda:updateSettings(master,rectification.get(),disparity.get(),flash.get(),exposure.get(),root))
